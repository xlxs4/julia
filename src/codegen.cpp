...
static std::pair<Function*, Function*> get_oc_function(jl_codectx_t &ctx, jl_method_t *closure_method, jl_tupletype_t *env_t, jl_tupletype_t *argt_typ, jl_value_t *rettype)
{
    ...
    if (ci == NULL || (jl_value_t*)ci == jl_nothing || jl_atomic_load_relaxed(&ci->inferred) == NULL || jl_atomic_load_relaxed(&ci->inferred) == jl_nothing) {
        JL_GC_POP();
            ...
            if (// keep code when keeping everything
                ...
                if (jl_atomic_load_relaxed(&codeinst->inferred) != (jl_value_t*)src) {
                    ...
                    jl_gc_wb(codeinst, src);
                     ...
                     (jl_atomic_load_relaxed(&codeinst->invoke) == jl_fptr_const_return_addr || !jl_ir_flag_inlineable((jl_array_t*)jl_atomic_load_relaxed(&codeinst->inferred))) &&
