...
struct StmtResult
    ...
    type::Any # ::Union{LatticeElement, Bool} - `false` if stmt may not be used as an SSA Value
    # effects
end

@inline function abstract_interpret_bb_stmt!(interp::AbstractInterpreter,
    ...
    if isa(stmt, NewvarNode)
        ...
        return StmtResult(changes, false)
...
@inline function propagate_exception_slots!(frame::InferenceState, changes::StateUpdate)
    let cur_hand = frame.handler_at[frame.currpc], l, enter
        while cur_hand != 0
            ...
            if stupdate1!(frame.bb_vartables[exceptbb]::VarTable, changes::StateUpdate) !== false ||
...
function typeinf_local(interp::AbstractInterpreter, frame::InferenceState)
    ...
    nbbs = length(frame.cfg.blocks)

    revisit = false

    if frame.currbb != 1
        frame.currbb = _bits_findnext(W.bits, 1)::Int # next program-counter
    end

    stoverwrite!(frame.pc_vartable, frame.bb_vartables[frame.currbb])
    while frame.currbb <= nbbs
        delete!(W, frame.currbb)
        frame.currpc = first(frame.cfg.blocks[frame.currbb].stmts)
        bbend = last(frame.cfg.blocks[frame.currbb].stmts)
        ...
        for frame.currpc in frame.currpc:bbend
            ...
            if frame.currpc == bbend
                ...
                if isa(stmt, GotoNode)
                    succs = frame.cfg.blocks[frame.currbb].succs
                    @assert length(succs) == 1
                    nextbb = succs[1]
                    ssavaluetypes[frame.currpc] = Any
                    @goto branch
                elseif isa(stmt, GotoIfNot)
                    condval, condt = abstract_interpret_branch_condition(interp, stmt, frame)
                    condt === Union{} && @goto find_next_bb
                    ssavaluetypes[frame.currpc] = Any
                    if condval === true
                        break
                    else
                        ...
                        if condval === false
                            ...
                            if newstate !== nothing || !was_reached(frame, first(frame.cfg.blocks[falsebb].stmts))
                                push!(W, falsebb)
                            end
                            break
                        end
                ...
                elseif isa(stmt, ReturnNode)
                    ssavaluetypes[frame.currpc] = Any
                    abstract_interpret_return(interp, nargs, stmt, frame)
                    @goto find_next_bb
            ...
            (; changes, type) = abstract_interpret_bb_stmt!(interp,
                frame.pc_vartable, stmt, frame)::StmtResult
            if type === Union{}
                @goto find_next_bb
            end
            if changes !== nothing
                stoverwrite1!(frame.pc_vartable, changes)
                propagate_exception_slots!(frame, changes)
            end
            if type === false
                ssavaluetypes[frame.currpc] = Any
                continue
    ...
    @label branch
        ...
        if newstate !== nothing || !was_reached(frame, first(frame.cfg.blocks[nextbb].stmts))
            push!(W, nextbb)
        end
        @goto find_next_bb
        ...
        if nextbb <= nbbs
            ...
            if newstate !== nothing
                frame.currbb = nextbb
                frame.currpc = first(frame.cfg.blocks[nextbb].stmts)
